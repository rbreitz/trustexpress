...
def get_difficulty_prediction():
    ...
    try:
        ...
        try:
            #try to pull reviews and prodcut info from dataframe
            product_info = get_product_info(product_url)
            product_reviews = get_product_reviews(product_info)
        except:
            #live scrape if information was not available
        ...
        main_rating = product_info['stars']
        number_reviews = product_info['votes']
        #need to update all of these - B
        try:
            product_photo_url = product_info['primary_image_url']
